...
class ModelModule(LightningModule):
    def __init__(self, args):
        ...
        if getattr(args, "pretrained_model_path", None):
            ...
            # Determine if it is a full Lightning checkpoint by checking for "state_dict" key
            ...
            if getattr(args, "transfer_frontend", False):
                # For transferring only the frontend component, use the keys starting with "trunk." or "frontend3D."
                tmp_ckpt = {k: v for k, v in state_dict.items() if k.startswith("trunk.") or k.startswith("frontend3D.")}
                ...
                # For transferring encoder components, apply a key mapping if necessary.
                tmp_ckpt_front = {k.replace("frontend.", ""): v for k, v in state_dict.items() if k.startswith("frontend.")}
                self.model.frontend.load_state_dict(tmp_ckpt_front, strict=False)
                tmp_ckpt_proj = {k.replace("proj_encoder.", ""): v for k, v in state_dict.items() if k.startswith("proj_encoder.")}
                self.model.proj_encoder.load_state_dict(tmp_ckpt_proj, strict=False)
                tmp_ckpt_enc = {k.replace("encoder.", ""): v for k, v in state_dict.items() if k.startswith("encoder.")}
                self.model.encoder.load_state_dict(tmp_ckpt_enc, strict=False)
                print("Pretrained weights of the frontend, proj_encoder and encoder components are loaded successfully.")
            else:
                # Full model: Remove the "model." prefix if present.
                new_state_dict = {}
                for key, value in state_dict.items():
                    # If the key has "model." as a prefix, remove it
                ...
                # Use strict=False to allow minor mismatches if necessary.
                self.model.load_state_dict(new_state_dict, strict=False)
                print("Pretrained weights of the full model are loaded successfully with key remapping.")
        else:
    ...
    def test_step(self, sample, sample_idx):
        x = self.model.frontend(sample["inputs"].unsqueeze(0))
        x = self.model.proj_encoder(x)
        enc_feat, _ = self.model.encoder(x, None)
        enc_feat = enc_feat.squeeze(0)
        nbest_hyps = self.beam_search(enc_feat)
        ...
        actual_token_id = sample["targets"]
        actual = self.text_transform.post_process(actual_token_id)

        # 🔤 Convert predicted tokens to string
        print("🧠 Predicted token IDs (after <sos>):", predicted_token_id)
        print("🔡 Post-processed prediction:", predicted)
        
        print("🎯 Actual targets token IDs:", actual_token_id)
        print("🔡 Post-processed actual:", actual)
        # print("Raw logits:", x)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if isinstance(batch_sizes, torch.Tensor):
            world_size = batch_sizes.numel()
            total_batch = batch_sizes.sum()
            scaling_factor = world_size / total_batch
            loss *= scaling_factor
